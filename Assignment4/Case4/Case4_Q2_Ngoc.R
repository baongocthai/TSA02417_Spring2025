...
lower <- c(-1,  rep(-1, 3),   -1,  log(1e-5), log(1e-5),  min(df$Y) - 20)
upper <- c( 1,  rep( 1, 3),   2,  log(1e+2), log(1e+2),  max(df$Y) + 20)
...
p1 <- ggplot(df_pred_1D, aes(x = time)) +
  ...
  labs(
    # title = "Observed vs Predicted Transformer Temperature: 1D state-space model",
...
#   -0.10, -0.80,
#   ## B 
#   0.1, 0.002, 0.2,     # first row (Ta, S, I)
#   -0.1, -0.002, -0.2,     # second row
#   ## C 
#   1,  -1,             # mostly first state
#   ## log σ's 
#   log(1), log(1), log(1),
#   ## x0 
#   df$Y[1], df$Y[1] - 1,
...
p1 <- ggplot(df_pred_2D, aes(x = time)) +
  ...
  geom_line(aes(y = y_preds, color = "Predicted"), size = 1, linetype = "dashed") +
  labs(
    # title = "Observed vs Predicted Transformer Temperature: 2D state-space model",
       y = "Temperature (°C)", color = "Legend") +
  theme_minimal() +
  theme(
    legend.position = c(0.1, 0.8),
...
## --- run the filter once, storing FILTERED states ----------------------
A <- matrix(par_hat[1:4],  2, 2, byrow = TRUE)
B <- matrix(par_hat[5:10], 2, 3, byrow = TRUE)
C <- matrix(par_hat[11:12], 1, 2)
...
x_filt <- matrix(NA_real_, nrow = nrow(df), ncol = 2)
P_est  <- diag(exp(par_hat[18])^2, 2)
x_est  <- matrix(par_hat[16:17], 2, 1)
...
ggplot() +
  ...
  geom_line(data = inputs_long,
            aes(time, scale(value),  linetype = Input), alpha = 1.5) +
